...
class _MyLevelPageState extends State<MyLevelPage> {
  ...
  // Level tiers with different colors
  final Map<int, LevelTier> levelTiers = {
    1: LevelTier(color: Colors.grey, name: 'Bronze', range: [1, 3]),
    2: LevelTier(color: Colors.blue, name: 'Silver', range: [4, 6]),
    3: LevelTier(color: Colors.purple, name: 'Gold', range: [7, 9]),
    4: LevelTier(color: Colors.amber, name: 'Diamond', range: [10, 10]),
  };
              ...
              Container(
                ...
                child: Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.military_tech_rounded, color: Colors.white, size: 40),
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      Text(
                        '$currentXP XP',
                        style: TextStyle(color: Colors.pink.shade300, fontSize: 14, fontWeight: FontWeight.w600),
                      ),
                      Text(
                        '$nextLevelXP XP',
                        style: TextStyle(
  ...
  Widget _buildLevelTile(int level) {
    LevelTier tier = _getLevelTier(level);
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: isUnlocked ? tier.color.withOpacity(0.2) : Colors.transparent,
            blurRadius: 15,
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    shape: BoxShape.circle,
                    color: isUnlocked ? tier.color.withOpacity(0.2) : Colors.white.withOpacity(0.1),
                    border: Border.all(color: tier.color, width: 2),
                  ),
                  child: Center(child: Icon(Icons.military_tech_rounded, color: tier.color, size: 24)),
                ),
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (!isUnlocked) // Add lock icon if locked
                      Padding(
                        padding: const EdgeInsets.only(left: 4),
                        child: Icon(Icons.lock_rounded, color: Colors.white.withOpacity(0.5), size: 16),
                      ),
                  ],
                ),
  ...
  Widget _buildHowToLevelUp() {
    final List<LevelUpMethod> methods = [
      ...
      LevelUpMethod(title: 'Buy Ride', icon: Icons.directions_car_rounded, color: Colors.purple),
      LevelUpMethod(title: 'Recharge Coin', icon: Icons.account_balance_wallet_rounded, color: Colors.amber),
    ];
  ...
  LevelTier _getLevelTier(int level) {
    for (LevelTier tier in levelTiers.values) {
      if (level >= tier.range[0] && level <= tier.range[1]) {
        return tier;
      }
    }
    return levelTiers[1]!; // Default to bronze
  }
...
class LevelTier {
  ...
  LevelTier({required this.color, required this.name, required this.range});
}
